...
class Linear(object):
    ...
    def pk(self, k, norm=5.0e6):
        ...
        T = self.ehu.T(k)
        Pk = k ** self.cosmo_pars['ns'] * T ** 2
        return Pk * norm

    def pkgen(self, log_kmin, log_kmax, nplk=160, npts=None, norm=5.0e6):
        """
        ...
        log_kmax: float
            log10 max k
        nplk=160: int
            the number of points per unit log(k), default 160.
            If npts is not sent, npts=(log_kmax-log_kmin)*nplk
        npts: float
            Number of points.  This will over-ride nplk.
        norm: float
            default =5.e6
        ...
        """
        if npts is None:
            npts = int((log_kmax - log_kmin) * nplk)
    ...
    def xi(self, rin, **keys):
        ...
        xi = xi * (self.cosmo_pars['sigma_8'] / s8) ** 2
    ...
    def m(self, r, B, **keys):
        ...
        """
        j3 = self.j3(r, **keys)
        return j3 * self.rhocrit0 * B * 1.0e12
    ...
    def xi2sigmavar(self, r, xi):
        ...
        You can calculate sigma_8 from the results
        ...
        tmp = int(ceil(log(rmax / rmin) / log(2)))
        num = 1 + nper2 * tmp
...
class EisenHu(object):
    ...
    def calc_constants(self):
        cpars = self.cosmo_pars
        bfrac = cpars['omega_b'] / cpars['omega_m']
        self.bfrac = bfrac

        Theta = self.Tcmb / 2.7

        Om = cpars['omega_m'] * cpars['h'] ** 2
        Ob = cpars['omega_b'] * cpars['h'] ** 2
        ...
        b2 = 0.238 * Om ** 0.223
        Zd = 1291 * Om ** 0.251 * (1 + b1 * Ob ** b2) / (1 + 0.659 * Om ** 0.828)  # noqa
        ...
        # Equation 7
        self.Ksilk = 1.6 * Ob ** 0.52 * Om ** 0.73 * (1 + (10.4 * Om) ** (-0.95))  # noqa
        ...
        # Equation 24
        self.betab = 0.5 + bfrac + (3 - 2 * bfrac) * sqrt(1.0 + (17.2 * Om) ** 2)  # noqa
    ...
    def T(self, kinput, retall=False):
        ...
        Tb = (
            To11 / (1.0 + (k * s / 5.2) ** 2)
            + self.alb
            / (1.0 + (self.betab / (k * s)) ** 3)
            * exp(-((k / self.Ksilk) ** 1.4))
        ) * jo
...
def test_xi_converge_nplk(epsfile=None):
    ...
    for nplk, color in zip(nplk_vals, color_vals):
        ...
        limxi = where(xi < 1.0e-5, 1.0e-5, xi)
